...
// Private study vault — for the owner only. Passcode via env (fallback for
// local). Combined with the noindex headers in vercel.json + robots.txt, the
// whole /bmla area is private and unlisted.
const PASS = import.meta.env.VITE_BMLA_PASSCODE || "killey-2026";

export default function BmlaGate({ children }) {
  const [ok, setOk] = useState(
    () => typeof window !== "undefined" && sessionStorage.getItem("bmla:v1:unlock") === "1",
  );
  ...
  return (
    <main className="relative flex min-h-screen items-center justify-center overflow-hidden px-6">
      ...
      <form onSubmit={submit} className="relative w-full max-w-sm rounded-xl2 border border-line/70 bg-surface/70 p-8 backdrop-blur">
        ...
        <p className="mt-1 text-sm text-muted">BMLA Mastery — for your eyes only. Enter your passcode.</p>
